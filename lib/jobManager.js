...
var JobManager = module.exports = function JobManager(){
    ...
    this.finalizeJob = function(jobId){
        if (this.validJobs[jobId]){
            delete this.validJobs[jobId];
        }
    }
    ...
    this.processShare = function(params, previousDifficulty, difficulty, remoteAddress, localPort){
        ...
        var hashBigNum = BigNumber(hash.toString('hex'), 16);

        var shareDiff = global.diff1Target.div(hashBigNum).toNumber();
        var foundBlock = false;

        //Check if share is a block candidate (matched network difficulty)
        if (job.target.gte(hashBigNum)){
            foundBlock = true;
        }
        else {
