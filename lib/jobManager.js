...
function JobManager(jobExpiryPeriod){
    ...
    function getWorkerAddress(worker){
        if (!isStringType(worker)){
            return null;
        }

        var index = worker.lastIndexOf('.');
        if (index === -1){
            return worker;
        }

        var prefix = worker.slice(0, index);
        var address = worker.slice(index + 1);
        if (prefix.length > 32){
            return null;
        }
        return address;
    ...
    this.processShare = function(params, previousDifficulty, difficulty, remoteAddress, localPort){
        ...
        var address = getWorkerAddress(params.worker);
        if (!address){
            return shareError([ErrorCodes.InvalidWorker, 'invalid worker']);
        }
        var [_, addressInvalid] = util.groupOfAddress(address)
        if (addressInvalid){
            return shareError([ErrorCodes.InvalidWorker, 'invalid worker']);
        }
