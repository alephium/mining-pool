...
var PaymentProcessor = module.exports = function PaymentProcessor(config, logger){
    ...
    this.prepareTransactions = function(fromAddress, fromPublicKey, utxos, balances){
        var txsData = [];
    ...
    function transferForGroup(balances, groupIndex, callback){
        ...
        _this.httpClient.getUtxos(fromAddress, function(result){
            ...
            var txsData = _this.prepareTransactions(fromAddress, fromPublicKey, result.utxos, balanceArray);
            prepareSendTxs(fromAddress, function(error){
                if (error){
                    callback();
                    return;
                }
                util.executeForEach(txsData, sendTx, _ => callback());
            });
        });
    }
    ...
    function sendTx(txData, callback){
    ...
    function transfer(allBalances){
        var allGroupBalances = _this.grouping(allBalances);
        util.executeForEach(
            allGroupBalances,
    ...
    this.start = function(){
        ...
        loadPublicKey(config.wallet, function(){
            setInterval(payment, config.paymentInterval * 1000);
        });
    }
