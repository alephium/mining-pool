...
var PaymentProcessor = module.exports = function PaymentProcessor(config, logger){
    ...
    this.prepareTransaction = function(fromPublicKey, utxos, balances){
        ...
        var txData = function(){
            if (txDestinations.length === 0 || txInputs.length === 0){
                ...
                destinations: txDestinations.map(e => ({address: e.address, attoAlphAmount: e.amount.toString()})),
    ...
    function transfer(addressInfo, groupedBalances, callback){
        ...
        _this.httpClient.getUtxos(fromAddress, function(result){
            ...
            var now = Date.now();
            var utxoIsEnough = haveEnoughUtxo(result.utxos, groupedBalances, now);
            if (!utxoIsEnough) {
                logger.error('Not enough utxo when transfer');
                logger.debug('Timestamp: ' + now + ', payment data: ' + JSON.stringify({utxos: result.utxos, groupedBalances: groupedBalances}));
                callback();
                return;
            }
    ...
    function haveEnoughUtxo(utxos, groupedBalances, now){
        var inputSum = 0, outputSum = 0;
        for (var utxo of utxos){
            if (utxo.lockTime <= now) {
