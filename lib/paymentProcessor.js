...
var PaymentProcessor = module.exports = function PaymentProcessor(config, logger){
    ...
    this.prepareTransactions = function(fromPublicKey, utxos, groupedBalances){
        var txsData = [];
        logger.debug('utxos and groupedBalances:' + JSON.stringify({utxos: utxos, groupedBalances: groupedBalances}));
        ...
        logger.debug('txs: ' + JSON.stringify(txsData));
        return txsData;
    }

    function transfer(groupedBalances, callback){
        var addressInfo = _this.addressInfo[0];
            ...
            prepareSendTxs(fromAddress, function(error){
                ...
                util.executeForEach(
                    ...
                    _ => {
                        ...
                        if (remains.length > 0){
                            logger.debug("Pending balances: " + JSON.stringify(remains));
                            delayTransfer(remains);
                            return;
                        }
                        callback();
                    }
    ...
    this.checkTxConfirmation = function(txId, callback){
        _this.httpClient.txStatus(txId, function(result){
            ...
            switch(result.type){
                case 'confirmed':
                    if ((result.chainConfirmations >= config.txConfirmations.chainConfirmations) &&
                        (result.fromGroupConfirmations >= config.txConfirmations.fromGroupConfirmations) &&
                        (result.toGroupConfirmations >= config.txConfirmations.toGroupConfirmations)
                    ){
                        logger.info('Tx ' + txId + ' confirmed');
                        _this.onTxConfirmed(txId, callback);
                        return;
                    }
                ...
                case 'mem-pooled':
                    ...
                    break;
                case 'not-found':
                    logger.info('Tx ' + txId + ' not found, unlock rewards');
                    _this.onTxFailed(txId, callback);
                    break;
                default:
    ...
    this.checkTxConfirmations = function(callback){
        _this.redisClient.smembers(transactionsKey, function(error, txIds){
            ...
            util.executeForEach(txIds, _this.checkTxConfirmation, _ => callback());
        });
    }
    ...
    function sweepToAddress(toAddress, fromAddress, callback){
        prepareSendTxs(fromAddress, function(error){
            ...
            _this.httpClient.sweepActiveAddress(
                ...
                function(result){
                    if (result.error){
                        logger.error('Sweep failed, error: ' + result.error + ', fromAddress: ' + fromAddress);
    ...
    function sweep(callback){
        var toAddress = config.addresses[0];
        var txs = [];
        util.executeForEach(config.addresses.slice(1),
            ...
            _ => {
                logger.debug('Sweep completed, pending txs: ' + JSON.stringify(txs));
                callback();
            }
        );
    }

    function delayTransfer(groupedBalances){
        setTimeout(_ => {
            transfer(groupedBalances, _ => {
    ...
    function payment(){
        ...
        _this.checkTxConfirmations(function(){
            _this.redisClient.hgetall(balancesKey, function(error, result){
                ...
                sweep(_ => delayTransfer(groupedBalances));
            });
        });
    }
