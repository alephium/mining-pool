...
var PaymentProcessor = module.exports = function PaymentProcessor(config, logger){
            ...
            _this.httpClient.buildUnsignedTxFromUtxos(
                ...
                function(unsignedTx){
                    ...
                    _this.httpClient.signTx(config.wallet.name, unsignedTx.txId, function(result){
                        ...
                        _this.submitTxAndUpdateBalance(signedTx, _ => callback());
                    })
                }
            )
        };
    ...
    this.submitTxAndUpdateBalance = function(signedTx, callback){
        _this.httpClient.submitTx(
            ...
            function(result){
                ...
                _this.updateBalances(signedTx.changedBalances, result.txId, callback);
            }
        );
    }

    _this.updateBalances = function(changedBalances, txId, callback){
        var commands = [];
        ...
        commands.push(['sadd', transactionsKey, txId]);
        _this.redisClient.multi(commands).exec(function(error, _){
            if (error){
