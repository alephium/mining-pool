...
var ShareProcessor = module.exports = function ShareProcessor(config, logger){
    ...
    this.handleShare = function(share){
        ...
        redisTx.hincrbyfloat(currentRound, share.worker, share.difficulty);

        var currentTs = Math.floor(currentMs / 1000);
        var difficulty = Math.min(share.shareDiff, share.difficulty * 10);
        redisTx.zadd(hashrateKey, currentTs, [fromGroup, toGroup, share.worker, difficulty, currentMs].join(':'));
        ...
        if (share.foundBlock){
            ...
            redisTx.hset(foundBlocksKey, blockHash, share.worker)
        }
    ...
    function blockInMainChain(blockHash, callback){
        _this.httpClient.getBlock(blockHash, function(block){
            if (block.error){
                callback(block.error, null, null);
                return;
            }
            _this.httpClient.blockHashesAtHeight(
                block.height,
                block.chainFrom,
                block.chainTo,
                function(result){
