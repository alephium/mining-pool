...
var StratumServer = exports.Server = function StratumServer(config){
    ...
    var _this = this;
    var stratumClients = {};
    var connectionNumFromIP = {};
    var subscriptionCounter = SubscriptionCounter();
    var bannedIPs = {};

    function addConnectionFromIP(ipAddress){
        var connectionNum = connectionNumFromIP[ipAddress];
        if (connectionNum){
            if (connectionNum >= config.maxConnectionsFromSameIP){
                return false;
            }
            connectionNumFromIP[ipAddress] += 1;
            return true;
        }
        connectionNumFromIP[ipAddress] = 1;
        return true;
    ...
    function removeConnectionFromIP(ipAddress){
        var connectionNum = connectionNumFromIP[ipAddress];
        if (connectionNum && connectionNum > 0){
            var num = connectionNum - 1;
            if (num == 0){
                delete connectionNumFromIP[ipAddress];
                return;
            }
            connectionNumFromIP[ipAddress] = num;
        }
    }
    ...
    this.handleNewClient = function(socket){
        if (!addConnectionFromIP(socket.remoteAddress)){
        ...
        client.on('socketDisconnect', function() {
            removeConnectionFromIP(client.remoteAddress);
